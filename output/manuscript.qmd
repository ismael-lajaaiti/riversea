...
- Community richness and hence food web structure is expected to be driven by nutrient availability [@Ho-Altermatt-Carraro-2023].
...
complementarity** metric following @Barnes-etal-2026.

**H1 — Nutrients have a unimodal effect on food web structure.**
Moderate nutrient enrichment should increase local productivity and prey
availability, supporting more diverse and complex food webs
[@Ho-Altermatt-Carraro-2023]. Beyond a threshold, excess nutrients are expected
to degrade food web structure through hypoxia, algal dominance, and turbidity
[@Ding-etal-2026; @Dean-etal-2023b]. This is consistent with the classic
*paradox of enrichment* [@Rosenzweig-1971], and with @Bonnaffé-etal-2024b's more
recent finding that richness and food-web structure jointly shape community
stability: enrichment destabilizes predator-prey dynamics, and the resulting
loss of destabilized interactions/species is one mechanism that could simplify
food web structure at high nutrient loads.

**H2 — Dam impact affects food web structure through (at least) two distinct
pathways, at two different spatial scales, which we predict act on different
metrics.** We distinguish a *local* pathway (distance to nearest dam) from a
*global* pathway (cumulative barrier effect / basin-wide fragmentation, per
AMOBIO's existing metrics). At the basin scale, cumulative fragmentation is
expected to reduce species richness and mean trophic level, through loss of
migratory and piscivorous species that depend on longitudinal connectivity
[@Dean-etal-2023b; @He-etal-2024]. At the same time, dams are known to
facilitate the spread of invasive species [@Johnson-Olden-VanderZanden-2008b],
including generalist invasive predators such as wels catfish (*Silurus glanis*)
— reservoirs can provide favorable larval-rearing habitat that promotes range
expansion, and the species is known to reshape recipient fish communities once
established [@Encina-etal-2024; @Bylak-etal-2023]. If such generalists
establish disproportionately in impounded reaches, we would predict connectance
to *increase* as richness and trophic complementarity trophic level decline.


**H3 — Part of the dam and nutrient effects on food web structure is mediated
by species richness, but we expect a direct effect too.** Consistent with the
DAG above, candidate direct-effect mechanisms — i.e. effects on structure that
don't operate simply by adding or removing species — in particular  the
compositional/identity effect described in H2, where an invasive generalist
changes connectance through its trophic role rather than through a change in
richness per se. Methodologically, we plan to fit two linked INLA models —
richness ~ dam + nutrients + spatial structure, and structure ~ dam + nutrients
+ richness + spatial structure — and compute the indirect effect as the product
of posterior samples of the relevant coefficients across the two fits. This
gives a fully Bayesian direct/indirect decomposition inside the INLA framework
itself, with the (Bayesian) SEM run alongside as an independent cross-check
rather than the sole source of the decomposition.

**H4 — A site's position along the river-to-sea continuum modulates its
sensitivity to both stressors, and we expect this sensitivity to peak in the
estuarine transition zone; both stressors may also propagate over long
distances rather than acting only locally.** Upstream (riverine) and downstream
(marine) sites differ systematically in baseline connectivity, salinity, and
community composition [@Whitfield-etal-2024; @Attrill-2002]. We predict that
the *slope* of the nutrient effect (and possibly the dam-impact effect) on food
web structure — i.e. an interaction between continuum position and stressor
magnitude — is not uniform along the continuum. Specifically, because estuaries
are naturally physiologically stressful environments (fluctuating salinity,
turbidity, low baseline diversity), resident communities may already sit closer
to their tolerance limits and hence be more sensitive to added anthropogenic
pressure [@Elliott-Quintino-2007] — consistent with synergistic rather than
additive multiple-stressor dynamics in already-stressed systems
[@Jackson-Pawar-Woodward-2021a]. This contrasts with a simple monotonic view in
which sensitivity would only increase or decrease steadily from headwater to
sea. Note this pairs naturally with the local/global distinction in H2: the
basin-wide barrier effect and nutrient loading are both plausibly
long-distance-acting pressures, which is part of why we model both a local
(distance) and a global (cumulative) dam metric rather than distance alone.

<!-- TODO on stress-gradient framing: the classic Stress Gradient Hypothesis
(facilitation replacing competition under high abiotic stress) is a different
mechanism than "already-stressed systems are more sensitive to an added
stressor" — worth treating as a conceptual parallel rather than a direct
citation match. More mechanistically apt candidates found via a quick search,
not yet in references.bib: a bioRxiv preprint "Ecological theory predicts
ecosystem stressor interactions in freshwater ecosystems" and "Interactive
effects of multiple stressors vary with consumer interactions, stressor
dynamics and magnitude" — both on non-additive/synergistic stressor effects in
freshwater systems specifically. Worth checking these against
@Jackson-Pawar-Woodward-2021a and @Orr-etal-2024a (already cited) for overlap
before adding. -->
