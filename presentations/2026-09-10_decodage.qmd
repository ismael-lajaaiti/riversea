---
title: "![](../assets/logo.svg){fig-alt=\"RIVERSEA\"}"
pagetitle: "RIVERSEA"
subtitle: "How damming and nutrient enrichment alter food web structure along the river-sea continuum"
author:
  - name: Ismaël Lajaaiti
    affiliations: INRAE, Rennes
  - name: Alain Danet
    affiliations: INRAE, Rennes
  - name: Anik Brind'Amour
    affiliations: Ifremer, Nantes
date: 09-02-2026
date-format: long
lang: en
bibliography: ../references.bib
format:
  revealjs:
    theme: brand
    fontsize: 32px
    footer: "[ismael-lajaaiti.github.io/riversea/](https://ismael-lajaaiti.github.io/riversea/)"
    slide-number: true
    transition: none
    incremental: false
    auto-stretch: false
    include-in-header:
      text: |
        <style>
        .reveal .subtitle { font-size: 1.6em; font-weight: 700; font-family: "Inter", sans-serif; }
        .reveal h1.title img { max-width: 40%; height: auto; }
        .reveal section[data-background-color] { color: #fff; }
        .reveal section[data-background-color] h1,
        .reveal section[data-background-color] h2 { color: #fff; }
        .reveal .figure-placeholder {
          height: 100%; min-height: 280px; max-height: 320px; display: flex;
          align-items: center; justify-content: center; text-align: center;
          border: 2px dashed #bbb; border-radius: 8px; color: #888;
          font-size: 0.7em; padding: 1em;
        }
        .reveal .credit {
          font-size: 0.5em; color: #888;
        }
        </style>
---

## A connected but siloed continuum

:::: {.columns}

::: {.column width="50%"}
- Rivers, estuaries and seas are **connected**
  - flux of nutrients, migration of species
- Yet they are often studied **in isolation**
  - freshwater vs marine ecology, separate surveys, ...
:::

::: {.column width="50%"}
![](assets/garonne-estuary.png)
:::

::: {.credit}
@Kavanaugh-etal-2013; Figure from CNES (2020)
:::

::::

## A connected but siloed continuum

:::: {.columns}

::: {.column width="50%"}
- Rivers, estuaries and seas are **connected**
  - flux of nutrients, migration of species
- Yet they are often studied **in isolation**
  - freshwater vs marine ecology, separate surveys, ...
- Cannot seize the impacts of pressures **across** ecosystems
  - Dams, nutrient enrichment
:::

::: {.column width="50%"}
![](../assets/stressors-cross-fields.png)
:::

::::

## Disturbances propagate at a distance


- Stressing one ecosystem can propagate to an other, **spatially distant**
ecosystem
- The **configuration of connectivity** mediates how the instability travels
- A local disturbance (dam, nutrient input) may therefore ripple along the
**whole** river-sea continuum 

![](../docs/assets/tadiri2024-results.png){fig-align="center"}

::: {.credit}
Figure from @Tadiri-etal-2024;
@McCann-etal-2021c
:::


## Taking a meta-ecosystem perspective

![](../assets/metaecosystem-diagram.png){fig-align="center" width="60%"}

- **Dams** block species migration
- **Nutrient runoffs** increase nutrient fluxes
- These stressors alter **how ecosystems are connected**

::: {.credit}
@Loreau-Mouquet-Holt-2003a; @Polis-Anderson-Holt-1997; @Gounand-etal-2014
@He-etal-2024
:::


## Capturing stressor impacts
### At a local scale

:::: {.columns}

::: {.column width="50%"}
- Representing the ecosystem as a food web (who eats whom)
- Enriching a local ecosystem can **destabilize** it rather than help
  - *Paradox of enrichment*: more resources, wilder predator-prey cycles
- We understand this **locally** -- but at the scale of a whole connected
continuum?
:::

::: {.column width="50%"}
![](../assets/foodweb.png){fig-align="center"}
:::

::::

::: {.credit}
@Gounand-etal-2014; @Rosenzweig-1971
:::

## Capturing stressor impacts
### At a global scale

Very few studies

- **@Bonnaffé-etal-2024** finds that nutrient and warming interact,
simplifying food web structure
  - but nutrients only (no dams), river only (no estuary, no sea)
- **@Carvajal-Quintero-etal-2026** finds that food web structure is changing
because of body size reduction
  - but no specific mechanistic driver
- We aim to resolve this with **clear mechanisms**, along the full
river-sea continuum, in France

::: {.credit}
@Bonnaffé-etal-2024; @Carvajal-Quintero-etal-2026
:::

# How do dams and nutrient enrichment alter food web structure along the river-sea continuum? {background-color="#275662"}

## Hypotheses

- **H1**: Pressures act in part through direct and richness-mediated effects
- **H2**: Nutrients have a unimodal effect on food web structure
- **H3**: Downstream, dams simplify food webs downstream, their effects fades
moving upstream

![](assets/fig-hypotheses.png){fig-align="center" width="70%"}

## Methods -- fish survey data

![](assets/fig-stations.png){fig-align="center" width="60%"}

- 3 surveys harmonized
  - ASPE (river, OFB), Pomet (estuary, INRAE), Nurse (sea, IFREMER)
- Species names, units and fish lengths standardized
- Missing sizes imputed per species from observed length distributions

## Methods -- reconstructing food webs

:::: {.columns}

::: {.column width="60%"}
- Diet table for 158 species
  1. `rfishbase` [@Boettiger-Lang-Wainwright-2012]
  2. validation with literature
- Metaweb built from diet table
- Subsample to get local webs
- Measure **connectance** & **trophic length**
:::

::: {.column width="40%"}
![](assets/fig-metaweb.png){fig-align="center" height="270"}
:::

::::

![](assets/fig-local-foodwebs.png){fig-align="center" height="220"}

## Methods -- environmental pressures

:::: {.columns}

::: {.column width="60%"}
- **Nutrients**: nitrate and phosphate
  - River: AMOBIO
  - Sea: REPHY, interpolated with `INLA` [@Lindgren-Rue-2015]
- **Dam impact**: mean dam height (AMOBIO)
  - Downstream: linear, single path to the river mouth
  - Upstream: dendritic, whole upstream network (heritage)
:::

::: {.column width="40%"}
![](assets/fig-rephy-mesh.png){fig-align="center"}
:::

::::

::: {.callout-warning}
Work in progress: nutrient definitions are not yet fully consistent
between AMOBIO and REPHY.
:::

## Methods -- position along the continuum

:::: {.columns}

::: {.column width="40%"}
- Distance to the nearest river mouth, computed for every fishing
  operation
  - **River**: shortest path along the hydrographic network
  - **Sea**: shortest path along a coastal mesh (`INLA`) excluding land

:::

::: {.column width="60%"}
![](assets/fig-distance-mouth.png){fig-align="center" width="80%"}
:::

::: {.callout-note}
We also added operation elevation and river width, when available.
:::

::::

## Methods -- statistical models

- Three Bayesian models (`R-INLA`): S, C, TL
- Testing the hypotheses
  - **H1**: species richness included as a covariate for C and TL
  - **H2**: quadratic terms for nitrate and phosphate
  - **H3**: dam barrier effects interact with distance to mouth
- Controlling for bias
  - Year and watershed random effects
  - Spatial autocorrelation: Gaussian random field (SPDE)

::: {.callout-warning}
Spatial autocorrelation uses euclidian and not hydrographic distance, for now.
:::


# Preliminary results -- Case study on the river continuum {background-color="#275662"}

## H1 - Testing richness mediated-effects

![](assets/fig-ame-main-effects.png){fig-align="center" width="70%"}

- Richness partly mediate stressor impact on structure *(validates H1)*
- Habitat variables strongly structure food webs
- Nutrients have opposite direct effects
- Dams have no **average** direct effects

## H2 - Testing nonlinear nutrient effects

![](assets/fig-ame-nutrients.png){fig-align="center" width="70%"}

- Nitrate has an unimodal effect on richness *(consistent with H2)*
- At high concentration
  - **Nitrate** increase is associated to **more complex** food webs
  - **Phosphate** increase is associated to **simpler** food webs

## H3 - Dam effects along the continuum

![](assets/fig-ame-dams-distance.png){fig-align="center" width="70%"}

- Dam effect depends on position along the continuum, in particular for food
web structure
- Close to river mouth
  - **Downstream dams** are associated with **simpler** food webs
  - **Upstream dams** are associated with **more complex** food webs

## Perspectives


#### Next steps

- Finish harmonizing environmental data across the continuum
- Extend statistical modeling to the full river–sea continuum

#### Further investigations

- Model spatial autocorrelation as a **Gaussian field on a graph**
  (`MetricGraph`), following hydrographic distance rather than Euclidean
  distance
- Refine dam impact metrics beyond height (e.g. fish passability, using
  ROE / Naiades)
- Make nutrient inference **consistent across river and coastal** data
  (e.g. AMOBIO vs REPHY nitrite/nitrate definitions)

# Thank you! {background-color="#275662"}

**Thanks to Bastien Bourillon, Olivier Dézérald and Karl Kreutzenberger for
their help with the AMOBIO dataset.**

*References*
