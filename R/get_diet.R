...
get_diet_category <- function(species) {
    ...
    diet_category <- diet_species |>
        ...
        select(Species, SampleStage, DietCode, FoodI, FoodII, FoodIII, ItemName, Stage, DietPercent) |>
        ...
        mutate(
            prey_category = case_when(
                ...
                FoodII == "other plants" & tolower(ItemName) == "macrophyte" ~ "macrophyte",
                FoodII == "other plants" & FoodIII == "terrestrial plants" ~ "macrophyte",
                FoodII == "other plants" & grepl("macrophyte", tolower(ItemName)) ~ "macrophyte",
