...
```{r}
fname <- here("data", "sea", "raw", "Reftax_SIH.txt")
...
```{r}
sea_data <- tar_read(sea_data_tidy)

data_solper <- sea_data$catch |> filter(survey == "solper")

d_solper <- data_solper |>
  left_join(solper_reftax, by = join_by(species == code)) |>
  select(-species) |>
  rename(species = scientific_name) |>
  mutate(species_valid = rfishbase::validate_names(species))

d_solper |> filter(is.na(species_valid))
