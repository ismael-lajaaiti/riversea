...
foodweb_long <- foodweb |>
  pivot_longer(
    cols = c(log_species_richness, connectance, trophic_length), names_to = "variable",
    values_to = "value"
  ) |>
...
```{r}
m_space <- vegan::rda(response ~ . + Condition(longitude + latitude),
  data = as.data.frame(predictor) |>
    bind_cols(fw_with_env |> select(longitude, latitude))
)
summary(m_space)

m_year <- vegan::rda(response ~ . + Condition(year),
  data = as.data.frame(predictor) |>
    bind_cols(fw_with_env |> select(year))
)
summary(m_year)
```
