...
### River-sea station map (report figure)

Same idea, but restricted to the four catchments actually kept for the
analysis (`district_kept`), with the river survey relabelled `ASPE` for
consistency with the progress-report text, main rivers added for spatial
context, and a temporal-coverage panel per survey. Built by
`plot_station_overview()` (`R/utils.R`); the progress report reads the
`plot_stations_overview` target directly, so there is no PNG to keep in
sync.
...
```{r}
env |>
  mutate(chlorophyll = chl_mean) |>
  filter(year == 2023) |>
  ggplot(aes(x = x, y = y, fill = chlorophyll)) +
  geom_tile() +
  geom_sf(data = coastline, inherit.aes = FALSE, linewidth = 0.3) +
  scale_fill_viridis_c() +
  coord_sf(xlim = range(env$x), ylim = range(env$y)) +
...
This seems to be due to an explosion in primary productivity values in recent
years. This is probably due to a change in the way the productivity is computed,
but let's further investigate.
...
We see indeed an explosion of the net primary productivity in the Gironde and
Loire estuaries.
