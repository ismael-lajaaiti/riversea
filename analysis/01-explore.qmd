...
pts_sites <- pts_vilaine.proj %>% left_join(site_vals, by = "pop_id")
```

```{r}
coords <- st_coordinates(pts_sites)
z <- pts_sites$mean_abundance
D.euclidian <- as.matrix(dist(coords))
n <- length(z)
# get upper triangle pairs
idx_pairs <- which(upper.tri(D.euclidian), arr.ind = TRUE)
dists <- D_euc[idx_pairs]
semivar_vals <- 0.5 * (z[idx_pairs[,1]] - z[idx_pairs[,2]])^2

# bin distances
nbins <- 15
breaks <- seq(0, max(dists), length.out = nbins+1)
bins <- cut(dists, breaks = breaks, include.lowest = TRUE)
gamma <- tapply(semivar_vals, bins, mean)
bin_mid <- (head(breaks,-1) + tail(breaks,-1))/2

plot(bin_mid, gamma, type="b", xlab="Euclidean distance (m)", ylab="Semivariance",
     main="Empirical semivariogram (Euclidean)")
```

### 4.3.4. Besag random effect
