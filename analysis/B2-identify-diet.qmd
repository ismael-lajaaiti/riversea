...
abundance_rank <- abundance_pomet |> group_by(n) |> summarise(count = n())
head(abundance_rank)
```
...
missing_pomet <- missing_pomet[!(missing_pomet %in% rare_pomet)]
```
...
missing_nurse <- missing_nurse[!(missing_nurse %in% rare_nurse)]
