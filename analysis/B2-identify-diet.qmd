...
Let's start with a single species for illustration.

```{r}
sp_test <- "Alosa alosa"
sp_test

diet_sp <- fooditems(sp_test)
diet_sp
```

```{r}
diet_sum <- diet_sp |> 
    group_by(PredatorStage) |> 
    select(Species, FoodI) |>
    distinct(Species, FoodI, PredatorStage) |>
    arrange(PredatorStage)
diet_sum
```

Let's do the same but for all species, all at once.

```{r}
diet_all <- fooditems(missing_all)
diet_sum <- diet_all |>
    group_by(Species, PredatorStage, PreyStage) |> 
    select(FoodI, FoodII) |>
    mutate(
        FoodI = case_when(
            FoodII == "phytoplankton" ~ "phytoplankton",
            FoodII == "other plants" & PreyStage == "leaves/blades" ~ "macrophyte",
            FoodII == "other plants" & PreyStage != "leaves/blades" ~ "biofilm",
            FoodI == "nekton" ~ "fish",
            TRUE ~ FoodI
        )
    ) |>
    ungroup() |>
    select(-PreyStage) |>
    distinct(Species, PredatorStage, FoodI) |>
    arrange(Species, PredatorStage)
diet_sum
```

Next, to obtain something similar as the table for riverine fish, we can pivot wider.
